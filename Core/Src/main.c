...
/* USER CODE BEGIN 0 */
	...
	static void FS_FileOperations(void) {
		if(f_mount(&USBHFatFS, (TCHAR const*)USBHPath, 0) == FR_OK) {
			...
			f_open(&MyFile, "STM32.TXT", FA_OPEN_ALWAYS);
		}
	}
/* USER CODE END 0 */
...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
	...
	BSP_LCD_DisplayStringAt(0, 50, (uint8_t *)"DATA MODE", CENTER_MODE); 
	BSP_LCD_DisplayStringAt(0, 75, (uint8_t *)"Plug USB", CENTER_MODE); 
...
//		BSP_LCD_DisplayStringAtLine(7, (uint8_t *)(char*)zPos);
      ...
      case APPLICATION_READY:
        FS_FileOperations();
				while(samples < 1000) {
					f_write(&MyFile, "Hello world", sizeof("Hello world"), (void *)&wbytes);
					samples++;
				}
